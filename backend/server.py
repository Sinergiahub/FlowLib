...
async def process_template_row(row: Dict[str, Any], action: str) -> Dict[str, Any]:
    ...
    try:
        ...
        template_data = {
            "slug": slug,
            "title": str(row.get('title', '')).strip() if row.get('title') and not pd.isna(row.get('title')) else '',
            "description": str(row.get('description', '')).strip() if row.get('description') and not pd.isna(row.get('description')) else None,
            "platform": str(row.get('platform', '')).strip() if row.get('platform') and not pd.isna(row.get('platform')) else '',
            "author_name": str(row.get('author_name', '')).strip() if row.get('author_name') and not pd.isna(row.get('author_name')) else None,
            "author_email": str(row.get('author_email', '')).strip() if row.get('author_email') and not pd.isna(row.get('author_email')) else None,
            "tutorial_url": str(row.get('tutorial_url', '')).strip() if row.get('tutorial_url') and not pd.isna(row.get('tutorial_url')) and str(row.get('tutorial_url', '')).strip() else None,
            "preview_image_url": str(row.get('preview_image_url', '')).strip() if row.get('preview_image_url') and not pd.isna(row.get('preview_image_url')) and str(row.get('preview_image_url', '')).strip() else None,
            "download_url": str(row.get('download_url', '')).strip() if row.get('download_url') and not pd.isna(row.get('download_url')) and str(row.get('download_url', '')).strip() else None,
            "json_url": str(row.get('json_url', '')).strip() if row.get('json_url') and not pd.isna(row.get('json_url')) and str(row.get('json_url', '')).strip() else None,
            "language": str(row.get('language', 'pt-BR')).strip(),
            "status": str(row.get('status', 'draft')).strip(),
            "tags": str(row.get('tags', '')).strip() if row.get('tags') and not pd.isna(row.get('tags')) and str(row.get('tags', '')).strip() else None,
            "notes": str(row.get('notes', '')).strip() if row.get('notes') and not pd.isna(row.get('notes')) and str(row.get('notes', '')).strip() else None,
            "external_id": str(row.get('external_id', '')).strip() if row.get('external_id') and not pd.isna(row.get('external_id')) and str(row.get('external_id', '')).strip() else None,
